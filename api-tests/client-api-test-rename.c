...
int main(int argc, char** argv) {
    ...
    printf("Test: rename file success");
    assert(tfsRename("a","c") == 0);
    printf("Test: New Already exist");
    assert(tfsRename("c","b") == TECNICOFS_ERROR_FILE_ALREADY_EXISTS);
    printf("Test: Old doesn't exist");
    assert(tfsRename("a","d") == TECNICOFS_ERROR_FILE_NOT_FOUND);
    assert(tfsUnmount() == 0);
    return 0;
}